...
public class OrderController {
  ...
  public String orderInfo(Authentication auth,
    ...
    try {
      ...
      OrderDTO resultOrderInfoDTO = orderService.orderInfo(memberDTO.getEmail(), productDTO.getProductId(),
              productNum, totalPrice, paymentMethod);
      session.setAttribute("orderInfoDTO", resultOrderInfoDTO);
      log.info("Controller_주문 정보 저장: {}", resultOrderInfoDTO);
      ...
      redirectAttributes.addFlashAttribute("searchUrl","/orderSuccess");
      return "redirect:/orderSuccess";
    } catch (Exception e) {
      // 주문 실패 시
      redirectAttributes.addFlashAttribute("error", "주문 실패하였습니다.");
      redirectAttributes.addFlashAttribute("searchUrl","/order?productId=" + productId);
  ...
  public String getOrderSuccess(ProductDTO productDTO, HttpSession session, Model model) throws Exception {
